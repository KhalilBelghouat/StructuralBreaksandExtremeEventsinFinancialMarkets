...
data <- merge(binary.dependent.variable, log.transaction.volume, exchange.rate, bloomberg.commodity.price.index, real.estate.sector, banking.sector, log.GPR, all = FALSE)
data <- as.data.frame(data)

# Model fitting.
fit <- glm(binary.dependent.variable ~ exchange.rate + bloomberg.commodity.price.index + transaction.volume + banking.sector + real.estate.sector + log.GPR, family = binomial(link = "logit"), data)
